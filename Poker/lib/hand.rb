...
class Hand
  ...
  def compare(other_hand)
    ...
    value == other_value ? break_tie(other_hand) : value <=> other_value
  end
  ...
  def break_tie(other_h)
    ...
    when 1
      ...
      other_sorted_values = other_h.cards.map { |card| card.value }.sort

      (0..4).each do |ranking|
        card_val = sorted_values[ranking]
        other_card_val = other_sorted_values[ranking]

        if (card_val <=> other_card_val) != 0
          return card_val <=> other_card_val
        end
      end
